...
function* getDivisors(value) {
    if (value <= 0)
        return;
    yield 1;
    const start = value % 2 !== 0 ? 2 : 3;
    const step = value % 2 !== 0 ? 1 : 2;
    for (let i = start; i < value / 2; i += step) {
        if (value % i === 0)
            yield i;
    }
    yield value;
...
if (cookieNumber.exists() && cookieDivisorsNumbers.exists()) {
    ...
    cookieForm?.addEventListener("submit", function (event) {
        ...
        const value = Number(cookieForm.inputNumber.value);
        if (value > MIN_NUMBER_LIMIT && value < MAX_NUMBER_LIMIT) {
            const stringifiedNumbers = JSON.stringify([...getDivisors(value)]);
...
if (savedStyle) {
    ...
    const savedRadio = form.querySelector(`input[value="${savedStyle}"]`);
    if (savedRadio)
...
class ImageManager {
    ...
    imageElement;
    constructor() {
        const formElement = document.getElementById("localStorageImageForm");
        if (!formElement) {
        ...
        this.imageBlockElement = imageBlockElement;
    ...
    addImageListenerHandler(event) {
        ...
        if (this.imageElement.files && this.imageElement.files.length > 0) {
            ...
            reader.onload = () => {
                ...
                if (base64String) {
                    const savedImages = localStorage.getItem(imageStorageName);
                    if (savedImages !== null) {
                        try {
    ...
    deleteImageListenerHandler(event) {
        event.preventDefault();
        const target = event.target;
